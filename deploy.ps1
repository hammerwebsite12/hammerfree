# Publish QuickPlay 2.4.0 installer to hammerwebsite12/hammerfree (quickplay branch)
...
Write-Host 'Creating GitHub release quickplay-v2.4.0...' -ForegroundColor Cyan
gh release view quickplay-v2.4.0 --repo hammerwebsite12/hammerfree 2>$null
if ($LASTEXITCODE -eq 0) {
    gh release upload quickplay-v2.4.0 QuickPlay.zip --repo hammerwebsite12/hammerfree --clobber
} else {
    gh release create quickplay-v2.4.0 QuickPlay.zip `
        --repo hammerwebsite12/hammerfree `
        --title "QuickPlay v2.4.0" `
        --notes "QuickPlay 2.4.0 — Anker gate CDN fix, Server 2 hidden from UI, auto-failover disabled. PyArmor-protected."
}
