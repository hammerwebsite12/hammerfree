<#
    QuickPlay 2.6.7 - One-paste installer
...
function Get-FileCurl([string]$url, [string]$dest) {
    ...
    & curl.exe -fL --retry 3 --retry-delay 5 -A 'QuickPlayInstaller/2.6.7' -o $dest $url 2>&1 | Out-Null
    ...
    for ($try = 1; $try -le $maxTries; $try++) {
        foreach ($url in $urlList) {
            ...
            try {
                ...
                $req.UserAgent        = 'QuickPlayInstaller/2.6.7'
...
try {
    ...
    if ($zipInfo.Length -lt $MinZipBytes -or $zipInfo.Length -gt $MaxZipBytes) {
        ...
        $msg = 'Downloaded payload looks wrong ({0} MB). Expected QuickPlay v2.6.7 (~26 MB). Try again or install from: https://github.com/{1}/releases/tag/{2}' -f $zipMb, $Repo, $ReleaseTag
        throw $msg
    }
    ...
    if ($exeInfo.Length -lt $MinZipBytes -or $exeInfo.Length -gt $MaxZipBytes) {
        ...
        $msg = 'Installed QuickPlay.exe looks wrong ({0} MB). Expected QuickPlay v2.6.7 (~26 MB).' -f $exeMb
        throw $msg
    }
