...
$ExtraDirs   = @('cache', 'specialpatch_cache')
...
function Get-FileCurl([string]$url, [string]$dest) {
    ...
    & curl.exe -fL --retry 3 --retry-delay 5 -A 'ValveOFFInstaller/1.0' -o $dest $url 2>&1 | Out-Null
    ...
    for ($try = 1; $try -le $maxTries; $try++) {
        foreach ($url in $urlList) {
            ...
            try {
                ...
                $req.UserAgent        = 'ValveOFFInstaller/1.0'
